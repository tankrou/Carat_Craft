...
// 2) Get the event to edit
$id = $_GET["id"];

// 3) Get the event record
$getRecord = $connection->prepare('SELECT * FROM event WHERE event_id = ?');
$getRecord->execute([$id]);
$getRecordResult = $getRecord->fetch();

if ($getRecordResult) {
    $event_id = $getRecordResult["event_id"];
    $event_name = $getRecordResult["event_name"];
    $event_desc = $getRecordResult["event_desc"];
    $event_date = $getRecordResult["event_date"];
    $event_venue = $getRecordResult["event_venue"];
}

if (isset($_POST["edit"])) {
    $event_name_new = $_POST["event_name"];
    $event_desc_new = $_POST["event_desc"];
    $event_date_new = $_POST["event_date"];
    $event_venue_new = $_POST["event_venue"];

    // Prepare update
    $sql = "UPDATE event SET event_name = ?, event_desc = ?, event_date = ?, event_venue = ? WHERE event_id = ?";
...
<div class="container user-profile-box mt-4">
    ...
    <form method="post">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="event_desc" rows="4" required><?= htmlspecialchars($event_desc) ?></textarea>
        </div>

        <div class="mb-3">
            <label class="form-label">Event Date</label>
            <input type="date" class="form-control" name="event_date" value="<?= htmlspecialchars($event_date) ?>" required>
        </div>

        <div class="mb-3">
            <label class="form-label">Event Venue</label>
            <input type="text" class="form-control" name="event_venue" value="<?= htmlspecialchars($event_venue) ?>" required>
        </div>

        <input type="hidden" name="event_id" value="<?= $event_id ?>">

        <button type="submit" name="edit" class="btn search-btn" style="width:150px;">Update Event</button>
        <a href="user_profile.php" class="btn search-btn">Back</a>
